...
bool compare_id(book &a, book &b) { return a.ids < b.ids; }

bool compare_name(book &a, book &b) { return a.name_book < b.name_book; }
...
struct library_system
{
  ...
  int menu()
  {
    ...
    while (choice == -1)
    {
      ...
      if (!(cin >> choice))
      {
        // If input cannot be converted to integer, clear the input stream and
  ...
  void print_library_by_id()
  {
    ...
    {
      cout << "id = " << books[i].ids << " name = " << books[i].name_book
           << " total_quantity " << books[i].total_quantity
           << " total_borrowed " << books[i].total_borrowed << "\n";
    }
  }
  ...
  void print_users()
  {
    for (int i = 0; i < len_user; i++)
    {
      cout << "user " << users[i].name_user << " id " << users[i].id_user
           << " borrowed books ids: ";
